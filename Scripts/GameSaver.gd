...
export(float) var next_save_wait_time = 1.0
...
func LoadGame():
	...
	for node_path in data.keys():
		...
		for attribure in data[node_path]:
			if attribure == "_time":
				get_parent().get_node("GameData").offine_time = OS.get_unix_time() - int(data[node_path]['_time'])
				
			if attribure == "_golds_on_second":
				get_parent().get_node("GameData").golds_on_second = float(data[node_path]['_golds_on_second'])
			...
			if attribure == "_price":
				...
				for i in range(items.get_child_count()):
					...
					item.find_node("*BuyBtn").set_disabled(price > get_parent().get_node("GameData").gold)
	...
	get_parent().get_node("GameData").update_gold_label()
	get_parent().get_node("GameData").update_xp_label()
	pass